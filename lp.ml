...
(**********************************************************************
*string_of_sig:
* Converts a context to an LP signature.
**********************************************************************)
...
and mapName =
  ...
  fun n ->
    try
      List.assoc n !mappedNames
    with
      Not_found ->
        if List.mem n reserved then
          let n' = generate (strip n) in
          (addMapping n n';
          n')
        else if List.exists (String.contains n) invalidCharacters then
          let n' = generate (strip n) in
          (addMapping n n';
          n')
        else
...
(**********************************************************************
*mapDomainName:
* Maps Twelf domain names to LP module names.
**********************************************************************)
let mapDomainName n =
...
(**********************************************************************
*getTypeHead:
* All types should have rigid constant heads; return this one.
**********************************************************************)
...
(**********************************************************************
*elide:
* Given a term, returns it only if it isn't 'true'.
**********************************************************************)
let elide t =
  if t = top then
    None
